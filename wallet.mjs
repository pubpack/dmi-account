...
const CONN_TIMEOUT = process.env.CONN_TIMEOUT || 8000;
...
async function createWallet(account, proxyAddr) {
    ...
    if (proxyAddr) {
        proxyAgent = new HttpsProxyAgent(proxyAddr);
    }

    const controller = new AbortController();
    const tmid = setTimeout(() => controller.abort(), CONN_TIMEOUT);
